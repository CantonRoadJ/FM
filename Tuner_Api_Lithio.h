...
extern int FM_Tune_To(AR_TuningAction_t mode,	 U16 frequency );
extern int AM_Tune_To(AR_TuningAction_t mode,	 U16 frequency );
extern int APPL_Get_Operation_Status(TUNER_STATE *status);
extern int APPL_Set_ReferenceClock(U32 frequency, bool is_ext_clk);
extern int APPL_Activate(void);
extern int APPL_Get_Quality_Status(bool fm,U8 *status );
...
extern void APPL_PowerSwitch(int OnOff);

extern void Radio_ChannelEqualizer(bool enable);
