...
int devTEF668x_Radio_Get_Quality_Level (bool fm,U8 *status,int16_t *level)
{
	...
	if(r == SUCCESS){
		...
		*level = (int16_t)(((int16_t)Convert8bto16b(buf+2))/10);
        ...
        //qDebug("devTEF668x_Radio_Get_Quality_Level is ok");
        //qDebug("status = %d", *status);
        //qDebug("level = %d", *level);
    }
    else
    {
        //qDebug("devTEF668x_Radio_Get_Quality_Level is fail");
    }
...
*/
int devTEF668x_Radio_Get_Quality_Data (int fm,U8 *usn,U8 *wam,int16_t *offset)
{
	U8 buf[14];
	int r;
	int16_t temp;
